...
auto_Modal=auto%>%
  mutate(CSP=case_when(
    ...
    substr(CSP,1,1)=="8"||substr(CSP,1,1)=="9"~"Autre"
  ),CATEGORIE=case_when(
...
ByJob=auto_Modal%>%
  mutate(Categor=case_when(
    CSP %in% c("Artisans, Commerçants & Chef entreprise","Cadres","Employés","Ouvriers")~"Sedentaire",
    CSP %in% c("Agriculteur","Intérimaire et techniciens")~"Mobile"
  ))%>%
  group_split(Categor)
